...
export class CustomReportComponent implements OnInit {
  ...
  getAllBranches() {
    ...
    this.serv.endpoint = 'order-manager/branch?companyId=' + this.filterForm.get('company_id').value;
  ...
  getConsolidatedReports() {
    ...
    if (this.serv.notNull(formValue.startDate) && this.serv.notNull(formValue.endDate)) {
    ...
    this.serv.endpoint = 'account-manager/transaction/consolidated-report?requiredTypes=' + formValue.requiredTypes
                                                                + '&startDate=' + formValue.startDate
                                                                + '&endDate=' + formValue.endDate
                                                                + '&includeOrders=' + formValue.includeOrder;
    this.serv.get().subscribe((response: any) => {
      ...
      this.transactionList.forEach(elem => {
        ...
        }else if (elem.transactionType === 'purchase'){
      ...
      this.orderList.forEach(elem => {
        this.orderSummary.ordersTotal = this.orderSummary.ordersTotal + parseFloat(elem.orderAmount);
      });
    });
  }
